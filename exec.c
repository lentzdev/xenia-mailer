...
/* Try '.COM', '.EXE', and '.BAT', '.BTM' on current filename,
   modify filename if found. */
...
static int tryext (char *fn)
{
   ...
   if (exists (fn))
      return 2;
   strcpy (ext, ".BTM");					/*AGL:17nov93*/
   if (exists (fn))						/*AGL:17nov93*/
      return 2; 						/*AGL:17nov93*/
   *ext = 0;
   return 0;
}
...
static int findfile (char *fn)
{
   ...
   if (isslash (fn [0]) && isslash (fn [1]))
      { 		    /* AGL: what's he upto here?? Forgot something!? */
      }
   ...
   hasext = (check & HAS_EXT) ? 				/*AGL:17nov93*/
	    ((!stricmp (ext, ".bat") || !stricmp(ext,".btm")) ? 2 : 1) : 0;
...
static void getcmdpath (void)
{
   ...
   if (!found)
      ...
      found = findfile (cmdpath);
      if (!found)						/*AGL:17nov93*/
	 {							/*AGL:17nov93*/
	 strcpy (cmdpath, "4DOS.COM");				/*AGL:17nov93*/
	 found = findfile (cmdpath);				/*AGL:17nov93*/
	 if (!found)						/*AGL:17nov93*/
	    cmdpath [0] = 0;					/*AGL:17nov93*/
	 }							/*AGL:17nov93*/
      } 							/*AGL:17nov93*/
}
   ...
   if (!spwn)
	 ...
	 if (spwn & USE_FILE)
	    ...
	    if (!tempdir (swapfn))
	       ...
	       sprint(&swapfn[strlen(swapfn)],"XENIA%02x$.SWP",task);
	       idx = strlen (swapfn) - 1;
	       while (exists (swapfn))
		  {
		  if (swapfn [idx] == 'Z')
